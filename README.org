...
** Introduction
   The Language and Voice Laboratory (HR) runs a tiny computing "cluster" called
   Terra.  This cluster consists of a few physical nodes, =terra=, =torpaq= and
   =gaia=.

** Getting access and logging in
   Access is granted by request by a sysadmin in the LVL.  

* Scheduler
  The LVL cluster uses [[https://slurm.schedmd.org][Slurm]] to handle compute job scheduling and resource
  allocation.  All resource intesive tasks *must* use the scheduling system.

  The command [[https://slurm.schedmd.com/sbatch.html][=sbatch=]] is used to submit batch jobs to the scheduler. This is
  ...
  running jobs.
   ...
   - [[file:slurm-usage.org][Learn more]]
